...
export default function Log(props) {
    ...
    for (let i in props.notUnpaid) {
        if (props.notUnpaid[i].status == "in progress") {
            table.push(<tr><td>{props.notUnpaid[i].name}<sub>in progress</sub></td> <td>{props.notUnpaid[i].amount}</td> </tr>);

        } else {
            table.push(<tr>
                <td>{props.notUnpaid[i].name}</td>
                <td>{props.notUnpaid[i].amount}</td>
            </tr>);
        }
    }
    ...
    return (
        ...
        <div className={"logPageLower"}>
            <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48"><path d="M10 40v-5h6.85l8.9-22H18V8h20v5h-6.85l-8.9 22H30v5Z"/></svg>
            <svg xmlns="http://www.w3.org/2000/svg" height="48" width="48"><path d="M21.5 40V13H10V8h28v5H26.5v27Z"/></svg>
